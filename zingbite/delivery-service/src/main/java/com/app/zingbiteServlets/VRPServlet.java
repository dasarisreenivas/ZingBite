...
public class VRPServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            // Dynamically fetch from Python service first
            JsonObject pythonRes = fetchPythonRoute(riderLat, riderLon, restLat, restLon,
                custALat, custALon, custBLat, custBLon,
                VRPRouteOptimizer.weather, VRPRouteOptimizer.perishableLifo, useAStar,
                prepTimeA, prepTimeB);

            List<Integer> sequence = new ArrayList<>();
            JsonObject pythonEtas = null;

            if (pythonRes != null && pythonRes.has("sequence") && pythonRes.has("predictiveETAs")) {
                JsonArray seqArr = pythonRes.getAsJsonArray("sequence");
                for (int i = 0; i < seqArr.size(); i++) {
                    String name = seqArr.get(i).getAsString();
                    if ("Customer A".equalsIgnoreCase(name)) {
                        sequence.add(2);
                    } else {
                        sequence.add(3);
                    }
                }
                pythonEtas = pythonRes.getAsJsonObject("predictiveETAs");
            } else {
                // Fallback to Java sequence optimization
                List<Integer> dropNodes = new ArrayList<>();
                dropNodes.add(2); // Customer A
                dropNodes.add(3); // Customer B
                sequence = optimizer.sequenceBatches(dropNodes);
            }
            ...
            if (pythonEtas != null) {
                try {
                    JsonObject etaA = pythonEtas.getAsJsonObject("Customer A");
                    JsonObject etaB = pythonEtas.getAsJsonObject("Customer B");
                    double pEtaA = etaA.has("total") ? etaA.get("total").getAsDouble() : etaA.getAsDouble();
                    double pEtaB = etaB.has("total") ? etaB.get("total").getAsDouble() : etaB.getAsDouble();
                    etaDrop1 = (firstDropId == 2) ? pEtaA : pEtaB;
                    etaDrop2 = (secondDropId == 2) ? pEtaA : pEtaB;
                } catch (Exception ex) {
                    etaDrop1 = optimizer.calculatePredictiveETA(distFM + distLM1, (trafficFM + trafficLM1) / 2.0, firstDropId == 2 ? prepTimeA : prepTimeB, firstDropId == 3);
                    etaDrop2 = optimizer.calculatePredictiveETA(distFM + distLM1 + distLM2, (trafficFM + trafficLM1 + trafficLM2) / 3.0, secondDropId == 2 ? prepTimeA : prepTimeB, secondDropId == 3);
                }
            } else {
                etaDrop1 = optimizer.calculatePredictiveETA(distFM + distLM1, (trafficFM + trafficLM1) / 2.0, firstDropId == 2 ? prepTimeA : prepTimeB, firstDropId == 3);
                etaDrop2 = optimizer.calculatePredictiveETA(distFM + distLM1 + distLM2, (trafficFM + trafficLM1 + trafficLM2) / 3.0, secondDropId == 2 ? prepTimeA : prepTimeB, secondDropId == 3);
    ...
    private JsonObject fetchPythonRoute(double riderLat, double riderLon, double restLat, double restLon,
                                         double custALat, double custALon, double custBLat, double custBLon,
                                         String weather, boolean perishableLifo, boolean useAStar,
                                         int prepTimeA, int prepTimeB) {
        try {
            JsonObject requestBody = new JsonObject();
            requestBody.addProperty("riderLat", riderLat);
            requestBody.addProperty("riderLon", riderLon);
            requestBody.addProperty("restLat", restLat);
            requestBody.addProperty("restLon", restLon);
            requestBody.addProperty("custALat", custALat);
            requestBody.addProperty("custALon", custALon);
            requestBody.addProperty("custBLat", custBLat);
            requestBody.addProperty("custBLon", custBLon);
            requestBody.addProperty("weather", weather);
            requestBody.addProperty("perishableLifo", perishableLifo);
            requestBody.addProperty("useAStar", useAStar);
            requestBody.addProperty("prepTimeA", prepTimeA);
            requestBody.addProperty("prepTimeB", prepTimeB);
            ...
            java.net.http.HttpRequest request = java.net.http.HttpRequest.newBuilder()
                    .uri(java.net.URI.create("http://localhost:5000/api/predict-route"))
            ...
            System.out.println("[VRPServlet] Python routing service unavailable, falling back to Java logic: " + e.getMessage());
        }
        return null;
    }
}
