...
public class DeliveryServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try (Session hibernateSession = DBUtils.openSession()) {
            ...
            responseJson.addProperty("riderStatus", user.getRiderStatus());
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if ("toggleAvailability".equals(action)) {
                ...
                try (Session hibernateSession = DBUtils.openSession()) {
                    Transaction txRider = hibernateSession.beginTransaction();
                    User dbUser = hibernateSession.get(User.class, user.getUserID());
                    if (dbUser != null) {
                        String currentStatus = dbUser.getRiderStatus();
                        if ("Active".equalsIgnoreCase(currentStatus)) {
                            newStatus = "Inactive";
                        } else {
                            newStatus = "Active";
                        }
                        dbUser.setRiderStatus(newStatus);
                ...
                user.setRiderStatus(newStatus);
                httpSession.setAttribute("loggedInUser", user);

                JsonObject jsonResponse = new JsonObject();
                jsonResponse.addProperty("success", true);
                jsonResponse.addProperty("riderStatus", newStatus);
                resp.getWriter().write(jsonResponse.toString());
                return;
            }
