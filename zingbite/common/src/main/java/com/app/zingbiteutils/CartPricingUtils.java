...
public final class CartPricingUtils {
    ...
    public static Totals calculateTotals(double subtotal, String rawCouponCode) {
        return calculateTotals(subtotal, rawCouponCode, 1.0);
    }

    public static Totals calculateTotals(double subtotal, String rawCouponCode, double surgeMultiplier) {
        ...
        double baseShipping = normalizedSubtotal == 0.0 || normalizedSubtotal >= FREE_SHIPPING_SUBTOTAL ? 0.0 : SHIPPING_FEE;
        double baseDeliveryFee = normalizedSubtotal == 0.0 ? 0.0 : SHIPPING_FEE;
        double surgeFee = baseDeliveryFee * (surgeMultiplier - 1.0);
        double shipping = baseShipping + surgeFee;
        ...
        return new Totals(normalizedSubtotal, shipping, tax, discount, total, coupon.valid ? coupon : null, surgeMultiplier, surgeFee);
    }
    ...
    public static final class Totals {
        ...
        public final double surgeFee;

        private Totals(double subtotal, double shipping, double tax, double discount, double total, Coupon coupon) {
            this(subtotal, shipping, tax, discount, total, coupon, 1.0, 0.0);
        }

        private Totals(double subtotal, double shipping, double tax, double discount, double total, Coupon coupon, double surgeMultiplier, double surgeFee) {
