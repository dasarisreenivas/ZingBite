...
public class SuperAdminServlet extends HttpServlet {
    ...
    private void clearJobsCache(HttpServletRequest req) {
        String serviceUrl = System.getenv().getOrDefault(
                "CAREERS_SERVICE_URL", "http://localhost:8085");
        HttpURLConnection connection = null;
        try {
            URL url = new URL(serviceUrl + "/api/careers");
            connection = (HttpURLConnection) url.openConnection();
            connection.setConnectTimeout(2000);
            connection.setReadTimeout(2000);
            connection.setRequestMethod("POST");
            ...
            // Write JSON payload: {"action": "clearCache"}
            String jsonPayload = "{\"action\":\"clearCache\"}";
            ...
            int status = connection.getResponseCode();
            if (status < 200 || status >= 300) {
                LOGGER.warn("[SuperAdmin] Jobs cache clear returned HTTP " + status);
            }
        } catch (Exception e) {
            LOGGER.warn("[SuperAdmin] Failed to clear jobs cache: " + e.getMessage());
        } finally {
            if (connection != null) {
                connection.disconnect();
            }
